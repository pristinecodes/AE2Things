...
public class Command {
    ...
    private static int help(CommandContext<ServerCommandSource> context) {
        context.getSource().sendFeedback(() -> Text.literal("Available Argument(s): "), false);
        context.getSource().sendFeedback(() -> Text.literal("/ae2things recover <UUID> - Spawns a drive with the given UUID, if it doesn't exist, does not spawn any item."), false);
        context.getSource().sendFeedback(() -> Text.literal("/ae2things getuuid - Gets the UUID of the drive in the player's hand if it has a UUID. Returns the DISKS uuid."), false);
        return 0;
    }
    ...
    private static int spawnDrive(CommandContext<ServerCommandSource> context, UUID uuid) throws CommandSyntaxException {
        ...
        if(AE2Things.STORAGE_INSTANCE.hasUUID(uuid)) {
            ...
            context.getSource().sendFeedback(() -> Text.translatable("command.ae2things.recover_success", player.getDisplayName(), uuid), true);
            return 0;
        }
        else {
            context.getSource().sendError(Text.translatable("command.ae2things.recover_fail", uuid));
            return 1;
        }
    }
    ...
    private static int getUUID(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        ...
        if(mainStack.getItem() instanceof IDISKCellItem) {
            if(mainStack.hasNbt() && mainStack.getNbt().contains(Constants.DISKUUID)) {
                ...
                context.getSource().sendFeedback(() -> Text.translatable("command.ae2things.getuuid_success", text), false);
                return 0;
            }
            else {
                context.getSource().sendError(Text.translatable("command.ae2things.getuuid_fail_nouuid"));
                return 1;
            }
        }
        context.getSource().sendError(Text.translatable("command.ae2things.getuuid_fail_notdisk"));
        return 1;
    }

    private static Text copyToClipboard(String string) {
        return Text.literal(string).styled(style -> style
